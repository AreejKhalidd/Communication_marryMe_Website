            ...
            <template>
              ...
              <v-list-item style="max-width: 1300px">
                ...
                <v-list-item-content style="text-align: right;margin: 0 20px 0 20px">
                  ...
                  <v-list-item-title class="font_name" style="font-weight: bolder">الأسم : {{ c.name }}</v-list-item-title>
                  <v-list-item-subtitle class="font_age" style="font-weight: bolder">العمر : {{ c.age }}</v-list-item-subtitle>
                </v-list-item-content>
                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn
                          ...
                          style="margin-left:  10px;background-color: green;font-weight: bolder;padding: 0 ;height: 25px;color: #EEEEEE;" depressed>
                        قبول
                      </v-btn>
                      <v-btn
                          v-bind="attrs"
                    ...
                    <v-card >
                      ...
                      <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn
                            @click="takeAction(c.id,2);dialog = false;" style="background-color: #FE6265"
                        >
                          رفض
                        </v-btn>
                        <v-btn
...
export default {
  ...
  methods: {
    ...
    reload(){
      ...
      axios.get("http://127.0.0.1:8000/api/admin/getAllNotCertifiedUsers", {headers: {Authorization: AuthStr}})
          .then(response => {
            // If request is good...
            this.certifies = response.data
            this.error = false;
            this.len = response.data.msg;
            console.log(this.certifies)
          })
          .catch(() => {
            this.error = true;
          });
