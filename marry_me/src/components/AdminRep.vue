<template>
  <v-card :id="'card'+id" style="background-color: white;margin:15px;direction: rtl" class="mx-auto">
      ...
      <template>
        ...
        <v-list-item style="max-width: 1300px">
          ...
          <v-list-item-content style="text-align: right;margin: 0 20px 0 20px">
            ...
            <v-list-item-title class="font_name" style="font-weight: bolder">رقم الرسالة : {{
                num
              }}
            </v-list-item-title>
            <v-list-item-subtitle class="font_age" style="font-weight: bolder">رقم البلاغ : {{
                id
              }}
            ...
            <v-list-item-subtitle :id="id +'data1'" class="font_age" style="font-weight: bolder;display: none">تم
              الانشاء في : {{ dateCreate }}
            </v-list-item-subtitle>
            <v-list-item-subtitle :id="id +'data2'" class="font_age" style="font-weight: bolder;display: none">تم
              النحديث في : {{ dateUpdate }}
            </v-list-item-subtitle>
            ...
            >
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                    v-bind="attrs"
                    v-on="on"
                    style="background-color: red;font-weight: bolder;padding: 0 ;height: 25px;color: #EEEEEE;"
                    depressed>
                  منع
                </v-btn>
                <font-awesome-icon
                    ...
                    @click="change(id)"
                    title="رؤية اكثر"
                    style="color: #0062CC;cursor: pointer;font-size: 37px;margin-right: 15px;margin-bottom: -8px"
                    :icon="showIcon"/>
                <font-awesome-icon
                    @click="reverseChange(id)"
                    :id="id+'show'"
                    title="رؤية اقل"
                    style="display: none;color: #0062CC;cursor: pointer;font-size: 37px;margin-right: 15px;margin-bottom: -8px"
                    :icon="lessIcon"/>
              ...
              <v-card>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn
                      @click="dialog = false;takeAction(id,3)" style="background-color: #FE6265"
                  >
                    منع
                  </v-btn>
                  <v-btn
                      @click="dialog = false;takeAction(id,4)"
                      color="primary"
                      text
